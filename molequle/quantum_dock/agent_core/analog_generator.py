...
def _apply_substitution(base_smiles: str, substitution_type: str, replacement: str) -> str:
    ...
    Args:
        ...
        substitution_type (str): Type of substitution (e.g., 'ligand_substitutions', 'functional_groups')
        replacement (str): Replacement group
        
    Returns:
        str: Modified SMILES string
    """
    # Enhanced substitution logic for cisplatin and general molecules
    logger = logging.getLogger(__name__)
    
    # For cisplatin (N[Pt](N)(Cl)Cl) and similar platinum complexes
    if "Pt" in base_smiles:
        cisplatin_substitutions = {
            "methyl": "N[Pt](NC)(Cl)Cl",  # Methyl substitution on one ammine
            "ethyl": "N[Pt](NCC)(Cl)Cl",  # Ethyl substitution on one ammine
            "hydroxyl": "N[Pt](N)(Cl)O",  # Hydroxyl substitution on one chloride
            "amino": "N[Pt](NN)(Cl)Cl",   # Additional amino group
            "carboxyl": "N[Pt](N)(Cl)OC(=O)C",  # Carboxyl substitution
            "phenyl": "N[Pt](Nc1ccccc1)(Cl)Cl",  # Phenyl substitution
            "benzyl": "N[Pt](NCCc1ccccc1)(Cl)Cl",  # Benzyl substitution
            "fluorine": "N[Pt](N)(F)F",   # Fluorine substitution
            "chlorine": "N[Pt](N)(Cl)Cl",  # Original (no change)
            "bromine": "N[Pt](N)(Br)Br",  # Bromine substitution
            "nitro": "N[Pt](N)(Cl)ON(=O)=O",  # Nitro substitution
            "sulfonyl": "N[Pt](N)(Cl)OS(=O)(=O)C",  # Sulfonyl substitution
            "ester": "N[Pt](N)(Cl)OC(=O)OC",  # Ester substitution
            "amide": "N[Pt](N)(Cl)NC(=O)C",  # Amide substitution
            "ether": "N[Pt](N)(Cl)OC"  # Ether substitution
        }
        
        if replacement in cisplatin_substitutions:
            return cisplatin_substitutions[replacement]
    
    # For general organic molecules (like CCO)
    else:
        organic_substitutions = {
            "methyl": "CCCO",  # Add methyl group
            "ethyl": "CCCO",   # Already ethyl-like
            "hydroxyl": "CCO",  # Already has hydroxyl
            "amino": "CCN",     # Amino substitution
            "carboxyl": "CC(=O)O",  # Carboxyl group
            "phenyl": "c1ccccc1CCO",  # Phenyl addition
            "benzyl": "c1ccccc1CCO",  # Benzyl addition
            "fluorine": "CCF",  # Fluorine substitution
            "chlorine": "CCCl",  # Chlorine substitution
            "bromine": "CCBr",  # Bromine substitution
            "nitro": "CCN(=O)=O",  # Nitro group
            "sulfonyl": "CCS(=O)(=O)C",  # Sulfonyl group
            "ester": "CCOC(=O)C",  # Ester group
            "amide": "CCNC(=O)C",  # Amide group
            "ether": "CCOC"  # Ether group
        }
        
        if replacement in organic_substitutions:
            return organic_substitutions[replacement]
    
    # If no specific substitution found, return original
    logger.warning(f"No substitution found for {substitution_type}:{replacement} on {base_smiles}")
    return base_smiles
...
def validate_molecule(mol_structure) -> bool:
    ...
    try:
        ...
        # Check for presence of platinum (preferred for cisplatin analogs, but not required)
        has_platinum = any(atom.GetSymbol() == 'Pt' for atom in mol_structure.GetAtoms())
        if not has_platinum:
            logging.getLogger(__name__).info("Molecule does not contain platinum - processing as general drug candidate")
            # Continue processing - platinum is preferred but not required
